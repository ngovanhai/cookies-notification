<?php
$select_settings = $dbr->query("SELECT * FROM shop_installed WHERE shop = '$shop' and app_id = $appId");
...
if ($installed) {
	...
	if ($result === false) {
		...
		if ($xml_doc->status == 'SUCCESS') {
			...
			$dbw->query("insert into test_data set test = 'QB Error is $xml_doc->errormessage'");
			//$db->query("delete from shop_installed where shop = '$shop' and app_id = $appId");
		}
	}
